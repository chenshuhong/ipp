...
// 	axios.post(opts,buffer.buffer,{
// 		headers:{
// 			'Content-Type':'application/ipp'
// 		},
// 		responseType:'arraybuffer'
// 	}).then(data=>{
// 		var response = parse(data.data);
// 		cb(null,response)
// 	}).catch(err=>{
// 		cb(err)
// 	})
...
module.exports = function(opts, buffer, cb,o){
	if(cb){
		console.log('cb')
		return wx.request({
			url:opts,
			...
			timeout:o&&o.timeout||60000,
			data:buffer.buffer,
			responseType:'ArrayBuffer',
			success:(data)=>{
				cb(null,parse(data.data))
			},
			fail:(err)=>{
				cb(err)
			}
