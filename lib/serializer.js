...
module.exports = function serializer(msg){
	...
	function writeStr(str, enc){
		var length = MyBuffer.byteLength(str);
	...
	function checkBufferSize(length){
		if (position + length > buf.length){
			buf = MyBuffer.concat([buf], 2 * buf.length);
		}
	}
	...
	if(typeof wx!=='undefined'){
		msg.data = MyBuffer.from(msg.data)
	}
	if(!MyBuffer.isBuffer(msg.data))
		throw "data must be a Buffer,gg"

	var buf2 = new MyBuffer(position + msg.data.length);
